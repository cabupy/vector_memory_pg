...
Estado: en progreso. Ya se agregaron al schema los campos `organization`, `project` y `repo_name`, con índice compuesto para namespace.
...
Estado: en progreso. Ya se agregó el campo `memory_type` al schema con valor por defecto `memory` e índice dedicado.
...
Estado: en progreso. Ya se agregó el campo `status` al schema con valor por defecto `active` e índice dedicado.
...
Estado: en progreso. Ya se agregaron al schema los campos `criticality` y `tags`, con índices dedicados. Quedan pendientes campos como `author`, `created_by_agent`, `source_commit`, `module`, `updated_at` y `last_accessed_at`.
...
- [ ] Conectar namespace, memory_type y status con ingesta/API/MCP.
```
...
- [ ] Conectar criticality y tags con ingesta/API/MCP.
```
